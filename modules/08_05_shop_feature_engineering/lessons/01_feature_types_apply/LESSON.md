...
|---|---|---|---|---|---|---|
| 1 | Три таблицы — три единицы наблюдения | 8 | Посчитайте строки и назовите единицу наблюдения каждой таблицы. | Задаёт вопрос о форме, ключе или диапазоне; не диктует строку кода | `## 1. Три таблицы — три единицы наблюдения` | assert раздела зелёный; ученик объясняет проверку |
| 2 | Числовые и категориальные столбцы | 10 | Соберите словарь типов вручную для четырёх бизнес-полей. | Задаёт вопрос о форме, ключе или диапазоне; не диктует строку кода | `## 2. Числовые и категориальные столбцы` | assert раздела зелёный; ученик объясняет проверку |
| 3 | Безопасный join заказов и оплат | 10 | Соедините по order_id слева и докажите сохранение строк. | Задаёт вопрос о форме, ключе или диапазоне; не диктует строку кода | `## 3. Безопасный join заказов и оплат` | assert раздела зелёный; ученик объясняет проверку |
| 4 | Функция категории оплаты | 10 | Напишите именованную функцию amount_band и проверьте границы 100 и 300. | Задаёт вопрос о форме, ключе или диапазоне; не диктует строку кода | `## 4. Функция категории оплаты` | assert раздела зелёный; ученик объясняет проверку |
| 5 | Apply к одному столбцу | 10 | Примените amount_band и проверьте покрытие всех строк. | Задаёт вопрос о форме, ключе или диапазоне; не диктует строку кода | `## 5. Apply к одному столбцу` | assert раздела зелёный; ученик объясняет проверку |
| 6 | Apply по строке | 10 | Рассчитайте days_to_deliver из двух дат с обработкой недоставленных заказов. | Задаёт вопрос о форме, ключе или диапазоне; не диктует строку кода | `## 6. Apply по строке` | assert раздела зелёный; ученик объясняет проверку |
| 7 | Apply или векторизация | 12 | Повторите расчёт векторно и сравните совпадение. | Задаёт вопрос о форме, ключе или диапазоне; не диктует строку кода | `## 7. Apply или векторизация` | assert раздела зелёный; ученик объясняет проверку |
| 8 | Карточка признака | 10 | Опишите источник, тип, правило, пропуски и риск для days_to_deliver. | Задаёт вопрос о форме, ключе или диапазоне; не диктует строку кода | `## 8. Карточка признака` | assert раздела зелёный; ученик объясняет проверку |
