...
|---|---|---|---|---|---|---|
| 1 | Контракт функции | 8 | Запишите сигнатуру и docstring, пока тело может быть заглушкой. | Задаёт вопрос о форме, ключе или диапазоне; не диктует строку кода | `## 1. Контракт функции` | assert раздела зелёный; ученик объясняет проверку |
| 2 | Копии и валидация | 10 | Реализуйте начало функции: копии, schema/key/value checks, первый лог. | Задаёт вопрос о форме, ключе или диапазоне; не диктует строку кода | `## 2. Копии и валидация` | assert раздела зелёный; ученик объясняет проверку |
| 3 | Форма результата | 10 | Проверьте одну строку клиента и семь ожидаемых полей. | Задаёт вопрос о форме, ключе или диапазоне; не диктует строку кода | `## 3. Форма результата` | assert раздела зелёный; ученик объясняет проверку |
| 4 | Инварианты RFM | 10 | Сверьте сумму заказов, денег и границы. | Задаёт вопрос о форме, ключе или диапазоне; не диктует строку кода | `## 4. Инварианты RFM` | assert раздела зелёный; ученик объясняет проверку |
| 5 | Негативный тест | 10 | Подайте отрицательную оплату и проверьте raise. | Задаёт вопрос о форме, ключе или диапазоне; не диктует строку кода | `## 5. Негативный тест` | assert раздела зелёный; ученик объясняет проверку |
| 6 | Повторяемость | 10 | Запустите дважды и сравните DataFrame и лог. | Задаёт вопрос о форме, ключе или диапазоне; не диктует строку кода | `## 6. Повторяемость` | assert раздела зелёный; ученик объясняет проверку |
| 7 | Preview артефакта | 12 | Сохраните первые 25 строк и прочитайте обратно. | Задаёт вопрос о форме, ключе или диапазоне; не диктует строку кода | `## 7. Preview артефакта` | assert раздела зелёный; ученик объясняет проверку |
| 8 | Acceptance и отчёт | 10 | Закройте пять критериев и напишите итог без обещания модели. | Задаёт вопрос о форме, ключе или диапазоне; не диктует строку кода | `## 8. Acceptance и отчёт` | assert раздела зелёный; ученик объясняет проверку |
