# Lesson Design: RFM через groupby

## A. Сценарий пары

| Поле | Значение |
|---|---|
| Модуль | Интернет-магазин / маркетплейс: Feature Engineering и lambda (`08_05`) |
| Название урока | RFM через groupby |
| Пара КТП | **34** |
| Длительность | 2 академических часа (**80 минут**) |
| Роль | введение |
| Пререквизиты | Python и pandas из модулей 1–3; три slim CSV рядом с ноутбуком; схема в [data/README](../../data/README.md) |
| **Открыть** | [lesson.ipynb](lesson.ipynb) — копия на ученика; первая code-ячейка загружает три CSV |
| **Первая фраза** | «Одна строка клиента получается не удалением дублей, а тремя осмысленными агрегатами» |
| **Минимум сдачи** | зелёные assert §§1–7; интерпретация §8; суммы Frequency и Monetary сходятся с источником |
| **Домашнее задание** | [homework.ipynb](homework.ipynb) — Part A обязательно; Challenge после основной части (~1 ч) |
| **Дальше** | [пара 35](../05_practice_aggregates/LESSON.md) |
| **Canvas** | не опубликовано |

### A. Чего хотим от пары

Главное — rfm переводит строки заказов в одну строку клиента по заранее зафиксированным агрегатам. Ученик не копирует готовую цепочку: на каждом этапе сначала
называет единицу наблюдения и ожидаемый инвариант, затем пишет код и использует assert
как доказательство. Бизнес-контекст — CRM Olist; метку churn не создаём и модель не обучаем.

Материал ноутбука: §1 Проверка ключей, §2 Таблица заказ+оплата, §3 Опорная дата, §4 Frequency, §5 Monetary, §6 Recency, §7 Сборка RFM, §8 Профиль и интерпретация. CSV лежат рядом, поэтому урок работает офлайн.

---

## B. Ход пары

| # | Этап | ~мин | Ученик | Учитель | Материал | Критерий закрытия |
|---|---|---|---|---|---|---|
| 1 | Проверка ключей | 8 | Проверьте уникальность ключей и покрытие оплат. | «Сколько уникальных customer_id ожидаете в итоге? Запишите до агрегации.» | `## 1. Проверка ключей` | assert раздела зелёный; ученик объясняет проверку |
| 2 | Таблица заказ+оплата | 10 | Сделайте one-to-one join. | «Проверьте: число строк после merge совпало с ожиданием? Если нет — где появились лишние?» | `## 2. Таблица заказ+оплата` | assert раздела зелёный; ученик объясняет проверку |
| 3 | Опорная дата | 10 | Используйте максимум даты всей таблицы. | «Почему нельзя взять опорную дату как максимум для каждого клиента отдельно?» | `## 3. Опорная дата` | assert раздела зелёный; ученик объясняет проверку |
| 4 | Frequency | 10 | Посчитайте число уникальных order_id на клиента. | «Почему nunique, а не count? Что считает count, чего не нужно?» | `## 4. Frequency` | assert раздела зелёный; ученик объясняет проверку |
| 5 | Monetary | 10 | Посчитайте сумму payment_value. | «Сумма M по всей таблице должна совпасть с суммой payments. Проверьте.» | `## 5. Monetary` | assert раздела зелёный; ученик объясняет проверку |
| 6 | Recency | 10 | Посчитайте дни от последней покупки до ref_date. | «Что должно быть у самого активного клиента — минимальный или максимальный Recency?» | `## 6. Recency` | assert раздела зелёный; ученик объясняет проверку |
| 7 | Сборка RFM | 12 | Объедините три Series в DataFrame. | «Сколько строк в итоге? Проверьте assert len(rfm) == 778.» | `## 7. Сборка RFM` | assert раздела зелёный; ученик объясняет проверку |
| 8 | Профиль и интерпретация | 10 | Получите describe и объясните высокий M при низком F. | «Это значит клиент купил мало, но дорого. Что это говорит о сегменте?» | `## 8. Профиль и интерпретация` | assert раздела зелёный; ученик объясняет проверку |

Обязательные этапы для минимума сдачи: **1–7**. §8 закрывает интерпретацию и переход к ДЗ.
Если этап занял больше времени, сокращать письменный объём заметки, но не убирать проверку данных.

---

## C. Если сбились

### Типичные ошибки

| Симптом / мысль ученика | Что сказать или показать |
|---|---|
| Recency равен нулю у всех | Не брать максимум отдельно внутри клиента как опорную дату. |
| Frequency считает строки после опасного join | Считать nunique order_id и сверить сумму. |
| Monetary превращает пропуски в незаметные нули | До groupby проверить покрытие оплат. |
| Высокий M называет лояльностью | Читать R, F и M вместе; это описание истории. |

### Дифференциация (кратко)

| | |
|---|---|
| Слабее базы | §§1–5 с парным проговариванием (формулируем агрегат до groupby); §§6–7 по шаблону со скаффолдом; ДЗ: только Part A §§1–2 |
| Сильнее базы | §§1–8 без подсказок; §8 — интерпретируют три нестандартных профиля из describe; Challenge: rfm_for_year(df, year) с параметром периода |

---

## D. Проектирование

### Зачем урок

RFM переводит строки заказов в одну строку клиента по заранее зафиксированным агрегатам. Без этой пары следующий шаг превращается в цепочку pandas-команд без контракта:
непонятно, что означает строка, почему сохранилось число объектов и доступен ли признак в момент решения.

### Центральная идея

| Поле | Значение |
|---|---|
| Центральная идея | RFM переводит строки заказов в одну строку клиента по заранее зафиксированным агрегатам. |
| Что поддерживает, но не отвлекает | Реальная схема Olist и CRM-интерпретация; predict и churn остаются вне scope |
| Данные урока | `orders_slim.csv`, `customers_slim.csv`, `payments_slim.csv`; правила RFM — [data/README](../../data/README.md) |

### Результаты обучения

1. Зафиксировать общую опорную дату Recency.
2. Вычислить Frequency как nunique заказов.
3. Вычислить Monetary как сумму оплат.
4. Собрать и проверить таблицу клиент×RFM.

### Профессиональный контекст

Это работа до обучения модели: аналитик превращает источники в воспроизводимую таблицу признаков,
фиксирует ключи, момент доступности, границы и аудит. Такой handoff можно проверять независимо от автора.

### Решения учащегося

| # | Какой выбор делает учащийся | На что влияет |
|---|---|---|
| 1 | Формулирует единицу наблюдения и ожидаемую форму | Корректность join/groupby и отсутствие размножения строк |
| 2 | Выбирает правило преобразования или агрегации | Интерпретируемость и воспроизводимость признака |
| 3 | Решает, какая проверка доказывает результат | Возможность заметить дефект до следующего этапа |

### Материалы (зачем каждый)

- [x] [lesson.ipynb](lesson.ipynb) — §§1–8, stubs и исполняемые assert
- [x] [homework.ipynb](homework.ipynb) — Part A + Challenge
- [x] [solutions.ipynb](solutions.ipynb) — секционные эталоны урока и ДЗ, только преподавателю
- [x] три slim CSV — автономный запуск без Kaggle
- [ ] Canvas — не опубликовано
- [ ] презентация — не нужна: ход и фиксации встроены в ноутбук

### Домашнее задание

| Поле | Значение |
|---|---|
| Назначается | **да** |
| Файл | [homework.ipynb](homework.ipynb) |
| Формулировка | Три задания Part A закрепляют вычисление и gate; Challenge переносит идею в функцию и письменное решение |
| Ориентир времени | **~1 ч** |
| Почему не на уроке | Самостоятельная функция и обоснование требуют повторного запуска без пошаговых подсказок |
| Какую способность развивает | Перенос контракта признака в новый, но структурно похожий сценарий |

---

## E. Карточка урока (§13)

| Поле | Значение |
|---|---|
| Часы | 2 |
| Стратегии обучения / виды деятельности | Guided coding → самостоятельная секция; прогноз формы до запуска; чтение assert как контракта |
| Формирующее оценивание | Зелёные assert §§1–7; объяснение одного инварианта и одного ограничения |
| Дифференциация (общая) | База: §§1–5 с парным формулированием агрегата (R/F/M); усиление: §8 (профили клиентов из describe) + Challenge rfm_for_year |
| По содержанию | RFM переводит строки заказов в одну строку клиента по заранее зафиксированным агрегатам. |
| По процессу | Индивидуальный код, короткие сверки после §§2, 5 и 7, взаимная проверка формулировки §8 |
| По продукту | Заполненный `lesson.ipynb`; сданный `homework.ipynb` Part A; Challenge по готовности |
| Canvas | не опубликовано |
