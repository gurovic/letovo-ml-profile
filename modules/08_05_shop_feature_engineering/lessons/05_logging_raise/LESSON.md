...
|---|---|---|---|---|---|---|
| 1 | Схема обязательных столбцов | 8 | Напишите require_columns с информативным KeyError. | Задаёт вопрос о форме, ключе или диапазоне; не диктует строку кода | `## 1. Схема обязательных столбцов` | assert раздела зелёный; ученик объясняет проверку |
| 2 | Уникальный ключ заказов | 10 | Напишите проверку дублей order_id. | Задаёт вопрос о форме, ключе или диапазоне; не диктует строку кода | `## 2. Уникальный ключ заказов` | assert раздела зелёный; ученик объясняет проверку |
| 3 | Неотрицательные оплаты | 10 | Остановите NaN и отрицательные значения. | Задаёт вопрос о форме, ключе или диапазоне; не диктует строку кода | `## 3. Неотрицательные оплаты` | assert раздела зелёный; ученик объясняет проверку |
| 4 | Обязательная дата покупки | 10 | Остановите NaT до расчёта Recency. | Задаёт вопрос о форме, ключе или диапазоне; не диктует строку кода | `## 4. Обязательная дата покупки` | assert раздела зелёный; ученик объясняет проверку |
| 5 | Единый валидатор | 10 | Соберите validate_inputs для трёх таблиц. | Задаёт вопрос о форме, ключе или диапазоне; не диктует строку кода | `## 5. Единый валидатор` | assert раздела зелёный; ученик объясняет проверку |
| 6 | Лог успешного пути | 10 | Запишите шаг только после успешной проверки. | Задаёт вопрос о форме, ключе или диапазоне; не диктует строку кода | `## 6. Лог успешного пути` | assert раздела зелёный; ученик объясняет проверку |
| 7 | Проверка плохого пути | 12 | Докажите, что после raise шаг merge не логируется. | Задаёт вопрос о форме, ключе или диапазоне; не диктует строку кода | `## 7. Проверка плохого пути` | assert раздела зелёный; ученик объясняет проверку |
| 8 | Инженерная записка | 10 | Объясните отличие assert, raise и лога. | Задаёт вопрос о форме, ключе или диапазоне; не диктует строку кода | `## 8. Инженерная записка` | assert раздела зелёный; ученик объясняет проверку |
