...
Вы — **стажёры агентства недвижимости**. Руководство дало таблицу квартир (площадь, цена, комнаты) и просит не «угадать в Excel», а собрать **прототип оценщика**: функции describe → transform → predict, метрика ошибки, выбор параметров по данным. Финал модуля — библиотека **`text_stats`**: сравнение частот слов в двух классах текстов (задел к будущему анализу таблиц и моделей).
