...
Главное — закрепить уже известные приёмы на серии задач: ещё рекурсия, короткая запись функции через `lambda` и вызов нескольких готовых функций по очереди. Новую большую тему не открываем.
