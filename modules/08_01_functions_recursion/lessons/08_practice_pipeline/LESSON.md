...
После введения рекурсии нужна **серия**, иначе приём не закрепится. Параллельно закрепляем короткий `lambda` в `filter`/`sorted` и сборку шагов в pipeline — без этого артефакт `text_stats` останется набором разрозненных функций.

Учим применять уже известные приёмы пачками и вызывать функции **по порядку** как один конвейер; новую теорию не разворачиваем.
