...
| **Открыть** | [lesson.ipynb](lesson.ipynb) — блоки A → B → C; данные: `NESTED_API_RESPONSE`, `MODEL_RUNS`, `FEATURE_ROWS` (доступ по ключу `['…']` — уже знакомый приём, не новая тема) |
...
Главное — закрепить уже известные приёмы на серии задач: ещё рекурсия, короткая запись функции через `lambda` и вызов нескольких готовых функций по очереди (`apply_pipeline`). Новую большую тему не открываем. Аббревиатура HOF в названии урока = «функция высшего порядка» (принимает другую функцию) — [UNIT §18](../../UNIT.md#18-глоссарий-модуля).

Побочно это готовит к итоговому файлу проекта: функции пока отрабатываем по отдельности, в один модуль их соберём на следующих парах.
...
| Симптом | Что сказать |
|---|---|
| `sorted(models)` без `key=` | «По какому полю сортировать? Нужен `key=lambda r: …`» |
| Scale до extract | Прогнать тип после каждого шага: сначала `extract_area(row)` → число, потом scale |
| Шаг без `return` | Печать типа после шага |
| «Разве dict не вне scope?» | Доступ `row['area_sqm']` / `r['f1']` — пререквизит (UNIT §4); теорию hash map не открываем |
...
| Формирующее оценивание | Assert A1, B1–B2, C1; прогон FEATURE_ROWS через pipeline |
| Дифференциация | Минимум A1–A2, B1–B2, C1; сильные — A3, B3, C2 |
| По содержанию | Только минимум серии vs углубление A3/B3/C2 |
| По процессу | Самостоятельная серия по блокам A→B→C; сверка assert |
| По продукту | Решённый минимум серии на паре; сдача `homework.ipynb` (A3, B3, C2) |
