...
Навыки модуля (tokenize, частоты, pipeline, рекурсия) должны сойтись в **один сдаваемый продукт** с автотестами — иначе остаётся набор упражнений без интеграции. Пара учит собирать библиотеку `text_stats` как инженерный артефакт: контракты функций + проверка тестами.

Сравнение классов текстов и README — на следующей паре; здесь фокус на сборке ядра.
