...
Главное — собрать функции, написанные за модуль, в один файл проекта и убедиться, что они проходят готовые автоматические тесты. Ученик работает по постановке: читает критерии, дописывает недостающие функции, запускает `manual_tests.py`. Новых конструкций языка не вводим.

Побочно сравнение двух наборов текстов и простую классификацию по частотам оставляем на следующую пару вместе с README и сдачей.
...
| По содержанию | Шаги 1–4 vs полный 1–5 + старт шага 6 |
| По процессу | Индивидуальная реализация по starter/README; тесты после группы функций |
