#!/usr/bin/env python3
"""Generate lesson notebooks for module 08_functions_recursion (data/ML integrated)."""
...
NOTEBOOKS = {
    ...
    "lessons/04_recursion/lesson.ipynb": nb(
        ...
        md("## 4. binary_search — рекурсивный поиск площади\n\nОтсортированный список площадей — быстрый поиск объекта."),
        code("def binary_search(sorted_areas, target, lo=0, hi=None):\n    \"\"\"Индекс target в sorted_areas или -1.\"\"\"\n    pass\n\n\nareas = sorted(a['area_sqm'] for a in APARTMENTS)\n# assert binary_search(areas, 45) == areas.index(45)\n"),
        md("## 5. count_char — базовая рекурсия на строке"),
        code("def count_char(s, ch):\n    pass\n\n\nassert count_char('banana', 'a') == 3\n"),
        md("## 6. Canvas: max digit в feature_id\n\nСтрока `'apt_128_floor_3'` — рекурсивно найти **максимальную цифру** (без `max()` по строке)."),
        code("def max_digit_recursive(s):\n    \"\"\"Max цифра в строке feature_id.\"\"\"\n    pass\n\n\nassert max_digit_recursive('apt_128_floor_3') == 8\n"),
        md("## 7. Fibonacci — антипример\n\nНаивная рекурсия vs итерация для n=25. Вывод: pipeline — итерация; рекурсия — для **структур данных**."),
        code("def fib_recursive(n):\n    if n <= 1:\n        return n\n    return fib_recursive(n - 1) + fib_recursive(n - 2)\n\n\ndef fib_iterative(n):\n    a, b = 0, 1\n    for _ in range(n):\n        a, b = b, a + b\n    return a\n\n\n# import time\n# сравните время для n=25\n"),
        md("## 8. Эксперимент\n\nСравните глубину рекурсии для `flatten(NESTED_API_RESPONSE)` и для `[[[[1]]]]`."),
        code("# наблюдения"),
        md("## 9. Самостоятельное задание\n\n`count_leaves(node)` — число «листьев» в дереве категорий (строки без вложенных списков).\n\nПодробнее: [canvas_practice.md](canvas_practice.md)."),
