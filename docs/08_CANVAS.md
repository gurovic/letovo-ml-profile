...
| Домашнее задание | **`Домашнее задание, урок N`** (Assignment; N — номер пары КТП; не отдельный ExternalUrl) |
| Опрос перед следующей парой | **`Опрос перед следующей парой`** (Quiz/survey; **скрыт**, пока не включают явно) |
...
Номер пары — в SubHeader (`Пара K. …`) и в названии Assignment ДЗ (`Домашнее задание, урок K`). У остальных элементов списка **не** дублировать «Пара K».

**Нет Assignment «Домашнее задание»** у пар **1** (ориентация) и **9–10** (артефакт). После элемента сдачи артефакта (конец модуля) **не** ставить ДЗ — сдача на паре 10, дальше домашнего задания модуля нет. **Опрос перед следующей парой** — у **каждой** пары, **последним материалом блока** (после ДЗ, если оно есть), включая ориентацию и артефакт.
...
| Slug wiki | `modul-N-sujet` при первом создании; Canvas может сменить url на translit после русского заголовка |
| Заголовок wiki | русский tagline из §1 — виден при открытии страницы |
| Тело wiki | текст «Описание сюжета» (без h2) |

Перед публикацией скрипт удаляет дубликаты и старые translit-slug. Карта фактических url — `modules/canvas_module_stories.json`.
...
**Перезалить уже опубликованную пару (8 класс):** `python scripts/publish_canvas_grade08_draft4.py --update-pairs 5 6` — перезаписывает файлы в том же gist (Colab-ссылки не меняются), план пары, заголовок блока «Пара N.» и текст ДЗ. Флаги публикации не трогает.
...
**Канон:** сразу после «Ноутбук урока» — один элемент **`Домашнее задание, урок N`** типа **Assignment** (N — номер пары КТП). Отдельного ExternalUrl на `homework.ipynb` в модуле **нет**.
...
| Название задания и элемента | **`Домашнее задание, урок N`** |
| Опубликован | **да** |
...
Переименовать уже залитые ДЗ: `python scripts/canvas_controller.py --rename-homework`

**API:**

```python
canvas_post(f"courses/{course_id}/assignments", {
    "assignment[name]": "Домашнее задание, урок 5",
...
canvas_post(f"courses/{course_id}/modules/{module_id}/items", {
    "module_item[title]": "Домашнее задание, урок 5",
...
### 11.3.3. Опрос перед следующей парой (Quiz / survey)

В конце **каждой** пары — один скрытый элемент **`Опрос перед следующей парой`** (Canvas Quiz, `quiz_type=survey`), **последним пунктом блока** (после «Домашнее задание», если оно есть).

| Параметр | Значение |
|---|---|
| Название | **`Опрос перед следующей парой`** |
| Тип | survey (без баллов) |
| Опубликован | **нет** (учитель включает, когда нужно) |
| Описание | «Заполните опрос после выполнения домашнего задания, но до начала следующего урока.» |
| Вопросы | (1) насколько материал был новым и сложным — шкала 1–5; (2) насколько понятно было на уроке / получилось выполнить ДЗ — шкала 1–5; (3) что ещё вы бы хотели сказать — свободный текст |

Добавить ко всем уже залитым парам:

```bash
python scripts/canvas_controller.py --add-feedback
```

При новой публикации пары опрос создаётся сам (`publish_canvas_*.py`).

Добавить к уже опубликованной паре Assignment «Домашнее задание»:
...
### 11.4. План пары (wiki для преподавателя)
...
**Для учителя на уроке (не весь файл):** в wiki только три блока — **«A. Чего хотим от пары»**, **B. Ход пары**, **C. Если сбились**. Без таблицы «Сценарий пары», без **D/E**, без **«Ориентир времени»** и без столбца **~мин** в ходе. Полный `LESSON.md` остаётся в репозитории. Ученические ноутбуки блок A **не** содержат.

Формулировки блока A (1–2 абзаца, главное/побочно, антипаттерны) — канон [Lesson Design § A. Чего хотим от пары](04_LESSON_DESIGN.md); здесь только состав тела wiki.

Элемент модуля «План урока (для преподавателя)» остаётся **не опубликован** (ученики не видят; учителя видят в модуле). Wiki-страница плана при `--update-page-only` **публикуется** (`published=true`), чтобы прямая ссылка и открытие из модуля работали без «страница недоступна».
...
- Канон slug: `wiki_page[title] = <slug>`; человеческое имя — только в `module_item[title]`.
