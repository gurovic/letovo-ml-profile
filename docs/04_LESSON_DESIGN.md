...
| **3. Материалы** | `lesson.ipynb` (если нужен); **обязательно** `homework.*` (~1 ч, уровни A–D); `solutions.ipynb` | [Notebook Standard](05_NOTEBOOK_STANDARD.md), `generate_notebooks.py` | Нет intro/итога в ноутбуке; данные inline; без `dict` до пререквизита; ДЗ **не** в `lesson.ipynb`; решения — для учителя |
