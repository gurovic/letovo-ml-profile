...
**1–2 абзаца** связного текста **только для учителя** (не телеграф и не список из двух ярлыков). В тексте должно быть ясно:

- чему учим **в первую очередь** (основная тема);
- что на паре есть **побочно** (не главная цель).

Без метафор. Общие ML-термины (модель, accuracy, MAE) допустимы, если из абзаца понятно, зачем они. Не подменять тему жаргоном ноутбука.
