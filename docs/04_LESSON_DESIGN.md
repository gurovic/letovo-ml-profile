...
1–2 коротких абзаца **только для учителя**. Без метафор.

Структура:

1. **Основная тема** — главная конструкция или умение пары.
2. **Побочно** — что ещё встречается, но не цель пары.

Язык: понятный учителю. **Общие ML-термины** (модель, accuracy, MAE, train/test…) допустимы. **Нельзя** писать внутренний жаргон ноутбука/пайплайна вместо темы (`describe` после scale, «контракт transform» как цель, если учим функциям Python).

Минимум сдачи и ход — в таблице A и в B. На Canvas — **«A. Чего хотим от пары»** ([08_CANVAS §11.4](08_CANVAS.md#114-план-пары-wiki-для-преподавателя)). «Ориентир времени» и «Сценарий пары» в план не входят.
