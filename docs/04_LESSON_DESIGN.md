...
Урок только с объяснением и одним демо — перепроектировать (MDS §5).
...
#### Пара 3 — отработка (`03_practice_predict`)

| Задача (зона B) | Заложено | Активная работа сильного ядра |
|---|---|---|
| `predict_pass` | 15 мин | 5–7 мин |
| `batch_predict` | 15 мин | 7–10 мин |
| `mae` | 20 мин | 7–10 мин |
| Выбор коэффициента | 20 мин | 5–10 мин (повтор с пары 2) |
| ДЗ-мост | 10 мин | 5 мин |
| **Итого** | **80** | **~30–40 мин** |

| Критерий | Оценка |
|---|---|
| Нагрузка идеи | низко-средняя: серия predict, batch, метрика — правильная педагогика |
| Нагрузка исполнения | низкая: шаблон задач очевиден из assert |
| Дублирование | задача 4 повторяет эксперимент пары 2 |
| Единственное нетривиальное расширение | `predict_room_price` — в домашнем задании, не на паре |

#### Сводка пилота

| | Пара 2 | Пара 3 | Для однородно сильного потока |
|---|---|---|---|
| Время | ~2× запас | ~2× запас | 60–80 мин на обе пары по минимуму, не 160 |
| Идеи | достаточно | достаточно | каркас верный, задачи не дотягивают |
| Действие при ревью | усилить исполнение на паре | убрать дубль MAE / усилить batch | зона C «сильнее базы» обязательна, не опция |

#### Направления усиления (без смены архитектуры модуля)

| Пара | Сильнее базы (зона C / ноутбук §7) |
|---|---|
| 2 | MAE считать **без** готового list comprehension; эксперимент — 5+ коэффициентов или таблица ошибок |
| 3 | `batch_predict` с проверкой длины/типа; сравнение порогов в `predict_pass` с обоснованием; подбор intercept и коэффициента вручную по `APARTMENTS` |
| Обе | обобщённая `evaluate(predict_fn, data)`; переписать batch через `map` |
...
Старые файлы (§1–§11 + «Для учителя») переводить в зоны A–E **по одному**, начиная с демо-модуля.
...
| Пара | Папка | Статус |
|---|---|---|
| 2 | `02_function_as_mapping` | пилот введение (Draft 5) |
| 3 | `03_practice_predict` | пилот отработка (Draft 5) |
| 1 | `01_intro_profile` | мигрировать |
| 3 | `03_practice_predict` | мигрировать |
| 4 | `04_parameters_and_return` | мигрировать |
| 5 | `05_practice_transform` | мигрировать |
| 6 | `06_scope_and_debugging` | мигрировать |
| 7 | `07_practice_metrics` | мигрировать |
| 8 | `08_recursion` | мигрировать |
| 9 | `09_practice_pipeline` | мигрировать |
| 10–11 | `10_artifact_build`, `11_artifact_submit` | мигрировать |
