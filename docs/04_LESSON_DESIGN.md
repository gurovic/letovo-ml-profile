...
1–2 коротких абзаца **только для учителя** (не в ученический ноутбук). Без метафор и «красот».

Структура:

1. **Основная тема** — чему учим в первую очередь (конструкции / идея).
2. **Побочно** — что появляется на паре, но не является главной целью.

Без чеклиста мини-артефактов и «успеха сдачи» (это в таблице A и в B). На Canvas — заголовок **«A. Чего хотим от пары»** в начале wiki «План урока (для преподавателя)» ([08_CANVAS §11.4](08_CANVAS.md#114-план-пары-wiki-для-преподавателя)). Блок «Ориентир времени» и таблица «Сценарий пары» в план не входят.
